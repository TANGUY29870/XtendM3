...
public class AddEcoProCod extends ExtendM3Transaction {
  ...
  public AddEcoProCod(MIAPI mi, DatabaseAPI database , ProgramAPI program, MICallerAPI miCaller, UtilityAPI utility) {
    this.mi = mi
  ...
  public void main() {
    ...
    if (isdf.equals("1") && !maintain_validateISDF(ecrg, csor, ecop)) {
      mi.error("Le code produit " + ecop + " est déjà le code produit par défaut dans l'éco-organisme " +
          ecrg + ". Un seul code produit peut être défini comme code par défaut.")
      return
    }
  ...
  // Un seul code produit peut être défini comme code par défaut pour la clé primaire n° 10
  private boolean maintain_validateISDF(String ecrg, String csor, String ecop) {
    boolean ok = true
    DBAction query = database.table("CECOPC").index("10").selection("CGTX40").build()
    DBContainer container = query.getContainer()
    ...
    Closure<?> releasedItemProcessor = {
      ...
      if (data.get("CGECRG") == ecrg && data.get("CGCSOR") == csor && data.get("CGECOP") != ecop && data.get("CGISDF") == 1) {
        ok = false
      } 
    }
